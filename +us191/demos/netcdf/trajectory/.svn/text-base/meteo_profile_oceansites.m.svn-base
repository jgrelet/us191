...
% nectdf descriptor
dynaload = 'meteo_trajectory_oceansites';
...
nc.ATTRIBUTES.type_instrument.data__    = 'BATOS';
nc.ATTRIBUTES.number_instrument.data__  = ' ';
...
nc.ATTRIBUTES.comment.data__            = 'Meteo data test from PIRATA-FR19 cruise';
...
% value SSTP  DRYT  WMSP  WDIR   ATMS  RELH
nc.VARIABLES.SSTP.data__        = medianf( SSTP', self.medianSize );
nc.VARIABLES.DRYT.data__        = medianf( DRYT', self.medianSize );
nc.VARIABLES.WSPD.data__        = medianf( WSPD', self.medianSize );
nc.VARIABLES.WDIR.data__        = medianf( WDIR', self.medianSize );
nc.VARIABLES.ATMS.data__        = medianf( ATMS', self.medianSize );
nc.VARIABLES.RELH.data__        = medianf( RELH', self.medianSize );
